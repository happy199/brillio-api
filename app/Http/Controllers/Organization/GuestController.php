...
class GuestController extends Controller
{
    ...
    public function update(Request $request, User $guest)
    {
        ...
        $validated = $request->validate($this->getValidationRules($guest->id));
    ...
    private function getValidationRules(?int $guestId = null): array
    {
        return [
    ...
    private function syncAcademicSteps(MentorProfile $profile, array $academicSteps, string $userName, string $context = 'Store')
    {
        if (empty($academicSteps)) {
            return;
        }

        // Trier par année pour être sûr de prendre la plus récente comme "actuelle"
        $steps = collect($academicSteps)->sortByDesc('year');
        $mostRecentStep = $steps->first();

        foreach ($academicSteps as $index => $step) {
            RoadmapStep::create([
