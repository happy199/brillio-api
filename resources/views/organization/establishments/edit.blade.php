...
@section('title', 'Modifier la fiche établissement')

@section('content')
<div class="max-w-5xl mx-auto py-8">
    <div class="mb-8">
        <a href="{{ route('organization.promotion.index') }}" class="text-organization-600 hover:text-organization-800 font-medium flex items-center gap-2">
            <i class="fas fa-arrow-left text-sm"></i> Retour au suivi de promotion
        </a>
        <h1 class="text-3xl font-bold text-gray-900 mt-4">Modifier la fiche établissement</h1>
        <p class="text-gray-500">Mettez à jour les informations qui seront affichées aux jeunes sur Brillio.</p>
    </div>

    <form action="{{ route('organization.establishments.update', $establishment) }}" method="POST" enctype="multipart/form-data" class="space-y-8" x-data="establishmentForm()">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $establishment->name) }}" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Type d'établissement</label>
                    <select name="type" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                ...
                <div>
                    ...
                    <input type="text" name="country" value="{{ old('country', $establishment->country) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Ville</label>
                    <input type="text" name="city" value="{{ old('city', $establishment->city) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Adresse physique</label>
                    <input type="text" name="address" value="{{ old('address', $establishment->address) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Email de contact</label>
                    <input type="email" name="email" value="{{ old('email', $establishment->email) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Téléphone</label>
                    <input type="text" name="phone" value="{{ old('phone', $establishment->phone) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-gray-700 mb-1">Description</label>
                    <textarea name="description" rows="4" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">{{ old('description', $establishment->description) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="p-6 space-y-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                ...
                                @if(isset($brochures[$i]))
                                    <a href="{{ Storage::url($brochures[$i]) }}" target="_blank" class="text-xs text-organization-600 font-medium hover:underline"><i class="fas fa-external-link-alt mr-1"></i> Fichier actuel ({{ basename($brochures[$i]) }})</a>
                                @endif
                            </div>
                            @endfor
                        </div>
                ...
                <div class="mt-6 border-t border-gray-100 pt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="url" name="website_url" value="{{ old('website_url', $establishment->website_url) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                    </div>
                    <div>
                        ...
                        <input type="url" name="google_maps_url" value="{{ old('google_maps_url', $establishment->google_maps_url) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">LinkedIn</label>
                        <input type="url" name="linkedin" value="{{ old('linkedin', $establishment->social_links['linkedin'] ?? '') }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Facebook</label>
                        <input type="url" name="facebook" value="{{ old('facebook', $establishment->social_links['facebook'] ?? '') }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Instagram</label>
                        <input type="url" name="instagram" value="{{ old('instagram', $establishment->social_links['instagram'] ?? '') }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6 space-y-6">
                <div>
                    ...
                    <div class="grid grid-cols-4 md:grid-cols-8 gap-2">
                        ...
                        <label class="relative flex flex-col items-center justify-center p-2 border-2 border-gray-100 rounded-xl cursor-pointer hover:bg-organization-50 transition"
                            :class="mbti_types.includes('{{ $type }}') ? 'border-organization-500 bg-organization-50' : ''">
                            <input type="checkbox" name="mbti_types[]" value="{{ $type }}" x-model="mbti_types" class="hidden">
                            <span class="text-xs font-bold" :class="mbti_types.includes('{{ $type }}') ? 'text-organization-700' : 'text-gray-400'">{{ $type }}</span>
                        </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="tuition_min" value="{{ old('tuition_min', $establishment->tuition_min) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Scolarité Max (CFA)</label>
                        <input type="number" name="tuition_max" value="{{ old('tuition_max', $establishment->tuition_max) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-organization-500 focus:border-organization-500">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                ...
                <label class="inline-flex items-center cursor-pointer">
                    ...
                    <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-organization-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:bg-organization-600 after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all"></div>
                        ...
                        <div class="flex items-start gap-2 bg-gray-50 p-4 rounded-xl border border-gray-100">
                            <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="text" :name="'precise_form_config['+index+'][label]'" x-model="field.label" required class="w-full text-sm border-gray-200 rounded-lg shadow-sm focus:ring-organization-500">
                                </div>
                                <div>
                                    <label class="block text-[10px] uppercase font-bold text-gray-400 mb-1">Type</label>
                                    <select :name="'precise_form_config['+index+'][type]'" x-model="field.type" class="w-full text-sm border-gray-200 rounded-lg shadow-sm focus:ring-organization-500">
                                ...
                                <div class="md:col-span-2" x-show="['select', 'radio', 'checkbox'].includes(field.type)">
                                    ...
                                    <input type="text" :name="'precise_form_config['+index+'][options]'" x-model="field.options_string" class="w-full text-sm border-gray-200 rounded-lg shadow-sm focus:ring-organization-500">
                                </div>
                            </div>
        ...
        <div class="flex items-center justify-between pt-4">
            <label class="inline-flex items-center">
                <input type="checkbox" name="is_published" {{ $establishment->is_published ? 'checked' : '' }} class="rounded border-gray-300 text-organization-600 shadow-sm focus:ring-organization-500">
                <span class="ml-2 text-gray-700 font-medium">Brouillon / Publié</span>
            </label>
            <button type="submit" class="px-10 py-4 bg-organization-600 text-white rounded-2xl font-bold text-lg hover:bg-organization-700 transition shadow-lg shadow-organization-200">
                Mettre à jour la fiche établissement
            </button>
        </div>
    </form>
</div>
