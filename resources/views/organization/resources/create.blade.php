...
@section('title', 'Nouvelle Ressource Interne')
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Nouvelle Ressource Interne</h1>
            <p class="text-sm text-gray-500">Créez et publiez une ressource pédagogique dédiée à vos jeunes.</p>
        </div>
    </div>
    ...
    <form action="{{ route('organization.resources.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3" required
                                ...
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-organization-500 focus:border-organization-500 block w-full p-3 transition resize-y">{{ old('description') }}</textarea>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4 shadow-sm">
                    ...
                    <div>
                        ...
                        <select name="type" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-organization-500 focus:border-organization-500 block w-full p-2.5">
                            ...
                            <option value="exercise" {{ old('type') === 'exercise' ? 'selected' : '' }}>📝 Exercice</option>
                            <option value="template" {{ old('type') === 'template' ? 'selected' : '' }}>📋 Modèle</option>
                            <option value="script" {{ old('type') === 'script' ? 'selected' : '' }}>📜 Script</option>
                            <option value="book" {{ old('type') === 'book' ? 'selected' : '' }}>📚 Livre / Support PDF</option>
                            ...
                            <option value="case_study" {{ old('type') === 'case_study' ? 'selected' : '' }}>📊 Étude de cas</option>
                            <option value="course" {{ old('type') === 'course' ? 'selected' : '' }}>🎓 Cours / Formation</option>
                        </select>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4 shadow-sm" x-data="{ isPremium: '{{ old('is_premium', '0') }}' }">
                    ...
                    <div x-show="isPremium === '1'" x-cloak class="pt-2">
                        ...
                        <input type="number" name="price" value="{{ old('price', 500) }}" min="200" step="50"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-organization-500 focus:border-organization-500 block w-full p-2.5">
...
<script>
    ...
    const oldQuizzes = {!! old('quizzes_data') ?: 'null' !!};
</script>
@endpush
