...
<div class="max-w-4xl mx-auto" x-data="{ 
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Programmer une séance</h1>
            <p class="mt-2 text-lg text-gray-600">Planifiez une session vidéo entre des intervenants et vos jeunes parrainés.</p>
        </div>
        <a href="{{ route('organization.sessions.calendar') }}" class="text-organization-600 hover:text-organization-700 font-semibold flex items-center transition-colors">
    ...
    @if($mentees->isEmpty())
        <div class="bg-indigo-50 border-2 border-indigo-100 rounded-2xl p-8 text-center shadow-sm">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-indigo-100 text-indigo-600 mb-4 text-2xl font-bold">!</div>
            <h3 class="text-xl font-bold text-gray-900 mb-2">Aucun jeune parrainé</h3>
            <p class="text-gray-600 mb-6 max-w-md mx-auto text-sm leading-relaxed">
                Vous devez avoir des jeunes parrainés pour pouvoir programmer une séance.
            </p>
            <a href="{{ route('organization.users.index') }}" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-bold rounded-xl text-white bg-indigo-600 hover:bg-indigo-700 transition-all shadow-md active:scale-95">
                Gérer mes jeunes
            </a>
        </div>
    @else
        <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
            <form action="{{ route('organization.sessions.store') }}" method="POST" class="p-8 space-y-8">
                @csrf
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                ...
                                class="w-full bg-gray-50 border-gray-200 rounded-xl focus:ring-organization-500 focus:border-organization-500 p-4 transition-all shadow-sm"></textarea>
                        </div>
                    </div>
                </div>
