...
<script nonce="{{ request()->attributes->get('csp_nonce') }}">
    window.showToast = function (message, type = 'success') {
        ...
        const parsedDoc = parser.parseFromString(icon, 'text/html');
        toast.appendChild(parsedDoc.body.firstElementChild);

        const textSpan = document.createElement('span');
        textSpan.className = 'font-medium text-sm text-gray-100';
