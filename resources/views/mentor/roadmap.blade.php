...
<div class="space-y-8" x-data="roadmapApp()">
    ...
    <div class="bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-200 rounded-2xl p-5">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="font-bold text-blue-900 line-clamp-1">Importez vos expériences LinkedIn</h3>
                    <p class="text-sm text-blue-700 line-clamp-1">Remplissez automatiquement votre parcours
                        professionnel</p>
                </div>
            </div>
            <button
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm">
        <div class="relative pl-20">
            <!-- Timeline Line -->
            <div class="absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-orange-500 via-red-500 to-pink-500">
            </div>

            <div class="space-y-8" id="roadmapSortable">
                @foreach ($steps as $index => $step)
                <div class="relative group" data-step-id="{{ $step->id }}">
                    <!-- Drag Handle -->
                    <div
                        class="absolute -left-16 top-6 cursor-move opacity-0 group-hover:opacity-100 transition p-2 bg-white rounded-lg shadow-sm">
                    ...
                    <div class="bg-gradient-to-br from-orange-50 to-red-50 rounded-xl p-5 hover:shadow-md transition">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <p class="text-orange-600 font-semibold">{{ $step->institution_company }}</p>
                                @endif
                                @if ($step->description)
                                <p class="text-gray-600 mt-2 text-sm leading-relaxed">{{ $step->description }}
                                </p>
                                @endif
                                ...
                                <div class="flex flex-wrap gap-2 mt-3">
                                    ...
                                    <span
                                        class="px-2 py-1 bg-white text-gray-600 text-xs rounded-full border border-gray-100 shadow-sm">{{
                                        $skill }}</span>
                                    @endforeach
                                </div>
                                @endif
                            ...
                            <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition">
                                <button @click="editStep({{ $step->id }})"
                                    class="p-2 bg-white/50 hover:bg-white rounded-lg transition shadow-sm"
                                    title="Modifier">
                                ...
                                <button @click="deleteStep({{ $step->id }})"
                                    class="p-2 bg-red-50 hover:bg-red-100 rounded-lg transition shadow-sm"
                                    title="Supprimer">
    ...
    <div x-show="showAddModal || showEditModal" x-cloak
        style="display: none;"
        class="fixed inset-0 z-[60] flex sm:items-center items-start justify-center p-4 bg-black/50 overflow-y-auto" x-transition>
