...
<div class="space-y-8" x-data="roadmapApp()">
        ...
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <p class="text-orange-700 text-sm mt-1">
                    Ajoutez les etapes importantes de votre parcours : formations, premiers emplois, promotions, projets cles...
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm">
        <div class="relative">
            <!-- Timeline Line -->
            <div class="absolute left-6 top-0 bottom-0 w-0.5 bg-gradient-to-b from-orange-500 via-red-500 to-pink-500"></div>
            ...
            <div class="space-y-6" id="roadmapSortable">
                ...
                <div class="relative pl-16 group" data-step-id="{{ $step->id }}">
                    ...
                    <!-- Timeline Dot -->
                    <div class="absolute left-4 w-5 h-5 bg-white border-4 border-orange-500 rounded-full transform -translate-x-1/2 mt-4"></div>

                    <div class="bg-gradient-to-br from-orange-50 to-red-50 rounded-xl p-5 hover:shadow-md transition">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="px-2 py-0.5 bg-orange-200 text-orange-800 text-xs font-medium rounded-full">
                                        Etape {{ $index + 1 }}
                                    </span>
                                ...
                                <p class="text-orange-600 font-medium">{{ $step->organization }}</p>
                                @endif
                                @if($step->description)
                                ...
                                <div class="flex flex-wrap gap-2 mt-3">
                                    ...
                                    <span class="px-2 py-1 bg-white text-gray-600 text-xs rounded-full border">{{ $skill }}</span>
                                    @endforeach
                                </div>
                                @endif
                            </div>
        ...
        <div class="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto" @click.outside="closeModals()">
            <div class="p-6 border-b sticky top-0 bg-white rounded-t-3xl">
                <div class="flex items-center justify-between">
                    <h3 class="text-xl font-bold text-gray-900" x-text="showEditModal ? 'Modifier l\'etape' : 'Ajouter une etape'"></h3>
            ...
            <form @submit.prevent="submitStep()" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" x-model="stepForm.skillsInput"
                    ...
                    <div class="flex flex-wrap gap-2 mt-2" x-show="stepForm.skills.length > 0">
                        <template x-for="(skill, index) in stepForm.skills" :key="index">
                            <span class="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-sm flex items-center gap-1">
...
function roadmapApp() {
    return {
        ...
        async editStep(stepId) {
            try {
                const response = await fetch(`/espace-mentor/roadmap/${stepId}`);
        ...
        async submitStep() {
            const url = this.showEditModal
                ? `/espace-mentor/roadmap/${this.editingStepId}`
                : '/espace-mentor/roadmap';
