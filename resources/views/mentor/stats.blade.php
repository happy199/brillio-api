...
@section('content')
    <div class="space-y-8" x-data="mentorStatsData()">
        <!-- Header -->
        <div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-3xl font-bold text-gray-900">
                            {{ $profile && $profile->is_published ? 'Publie' : 'Brouillon' }}
                        </p>
        ...
        @php
            $linkedOrg = $user->organization ?? $user->organizations()->first();
        @endphp
        @if($linkedOrg)
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-orange-200 mb-6">
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-gray-600 mb-4">
                        Vous êtes actuellement lié(e) à l'organisation <strong>{{ $linkedOrg->name }}</strong>. Cette organisation peut vous lier à des jeunes, suivre vos interactions (discussions, calendrier et visioconférence).
                    </p>
        ...
        <!-- Zone de Danger -->
        <div class="bg-white rounded-2xl shadow-sm p-8 border border-red-200">
        ...
        <!-- MODAL 1: Confirmation initiale de suppression -->
        ...
        <div x-show="showUnlinkConfirm" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" x-cloak
            ...
            <div class="bg-white rounded-3xl max-w-md w-full p-6" @click.away="showUnlinkConfirm = false">
                ...
                <p class="text-gray-600 text-center text-sm mb-6">
                    Un e-mail sera envoyé à l'organisation pour la notifier que vous avez décidé de rompre la relation de mentorat en tant que Mentor. Plus aucune information sur vous ne sera visible dans leur espace.
                    <br><br>
                ...
                <form action="{{ route('mentor.profile.unlink') }}" method="POST">
                    @csrf
                    <div class="flex gap-3">
    ...
    <script nonce="{{ request()->attributes->get('csp_nonce') }}">
        function mentorStatsData() {
            return {
                ...
                async generateCode() {
                    try {
                        const response = await fetch('{{ route("mentor.account.confirmation-code") }}');
                ...
                async submitArchive() {
                    ...
                    form.method = 'POST';
                    form.action = '{{ route("mentor.account.archive") }}';
