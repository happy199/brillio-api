...
<div class="container mx-auto px-4 py-8" x-data="{ activeTab: 'active' }">
    ...
    <div class="flex border-b border-gray-200 mb-6 overflow-x-auto">
        <button @click="activeTab = 'active'"
            ...
            <span class="bg-green-100 text-green-600 py-0.5 px-2.5 rounded-full text-xs font-bold">{{
                $activeMentees->count() }}</span>
            @endif
        </button>
        ...
        <button @click="activeTab = 'pending'"
            ...
            <span class="bg-indigo-100 text-indigo-600 py-0.5 px-2.5 rounded-full text-xs font-bold">{{
    ...
    <div x-show="activeTab === 'active'" x-transition.opacity>
        ...
        <div class="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-100">
            <div
                class="bg-indigo-50 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4 text-indigo-400">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col h-full">
                <div class="flex items-center gap-4 mb-6">
                    @php
                    $profileUrl = $mentorship->mentee->jeuneProfile && $mentorship->mentee->jeuneProfile->is_public ?
                    route('jeune.public.show', $mentorship->mentee->jeuneProfile->public_slug) : '#';
                    $isPublic = $mentorship->mentee->jeuneProfile && $mentorship->mentee->jeuneProfile->is_public;
                    @endphp
                    <a href="{{ $profileUrl }}"
                        class="{{ $isPublic ? 'hover:opacity-80 transition-opacity' : 'cursor-default' }}">
                        <img src="{{ $mentorship->mentee->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode($mentorship->mentee->name) }}"
                            alt="{{ $mentorship->mentee->name }}"
                            class="w-14 h-14 rounded-full object-cover bg-gray-100 border-2 border-indigo-100">
                    </a>
                    <div>
                        <a href="{{ $profileUrl }}"
                            class="{{ $isPublic ? 'hover:text-indigo-600 transition-colors' : 'cursor-default' }}">
            ...
            <!-- DISCONNECT MODAL -->
            <dialog id="disconnect-modal-{{ $mentorship->id }}"
                class="modal bg-white rounded-xl shadow-xl p-0 w-full max-w-md backdrop:bg-gray-900/50">
                <div class="p-6">
                    <h3 class="font-bold text-lg mb-4 text-gray-900">Mettre fin au mentorat ?</h3>
                    <p class="text-gray-600 mb-4 text-sm">Vous êtes sur le point de mettre fin à votre accompagnement
                        avec {{ $mentorship->mentee->name }}. Cette action est irréversible.</p>

                    <form action="{{ route('mentor.mentorship.disconnect', $mentorship) }}" method="POST">
                        @csrf
                        <textarea name="diction_reason" rows="4"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:border-red-500 focus:ring-red-500 mb-4 text-sm"
                            placeholder="Raison de la fin de l'accompagnement..." required></textarea>

                        <div class="flex justify-end gap-3">
                            <button type="button"
                                onclick="document.getElementById('disconnect-modal-{{ $mentorship->id }}').close()"
    ...
    <div x-show="activeTab === 'pending'" x-cloak>
        ...
        <div class="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-100">
            <div
                class="bg-gray-50 dark:bg-gray-800 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4 text-gray-400">
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Demande envoyée le {{ $mentorship->created_at->format('d/m/Y')
                            }}</p>
                    </div>
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <button onclick="document.getElementById('refuse-modal-{{ $mentorship->id }}').showModal()"
            ...
            <!-- REFUSE MODAL -->
            <dialog id="refuse-modal-{{ $mentorship->id }}"
                class="modal bg-white rounded-xl shadow-xl p-0 w-full max-w-md backdrop:bg-gray-900/50">
                <div class="p-6">
                    <h3 class="font-bold text-lg mb-4 text-gray-900">Refuser la demande ?</h3>
                    <p class="text-gray-600 mb-4 text-sm">Veuillez expliquer pourquoi vous ne pouvez pas accepter cette
                        demande. Ce message sera envoyé à {{ $mentorship->mentee->name }}.</p>

                    <form action="{{ route('mentor.mentorship.refuse', $mentorship) }}" method="POST">
                        @csrf
                        <textarea name="refusal_reason" rows="4"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 mb-4 text-sm p-3"
                            placeholder="Raison du refus..." required></textarea>

                        <div class="flex justify-end gap-3">
                            <button type="button"
                                onclick="document.getElementById('refuse-modal-{{ $mentorship->id }}').close()"
                                class="text-gray-500 hover:text-gray-700 text-sm font-medium px-4 py-2">Annuler</button>
                            <button type="submit"
                                class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium">Confirmer
                                le refus</button>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <span
                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Refusé</span>
                            @else
                            <span
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate"
                            title="{{ $item->refusal_reason ?? $item->diction_reason }}">
