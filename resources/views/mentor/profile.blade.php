    ...
    <form action="{{ route('mentor.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <div class="mb-8 flex flex-col sm:flex-row items-center gap-6">
                <div class="relative group">
                    <div class="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-lg bg-gray-100">
                        ...
                        @else
                        <div
                            class="w-full h-full flex items-center justify-center bg-gradient-to-br from-orange-400 to-red-500 text-white text-3xl font-bold">
                ...
                <div class="flex-1 text-center sm:text-left">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Photo de profil (Obligatoire pour
                        publier)</label>
                    <p class="text-xs text-gray-500 mb-3">Une photo rassure les jeunes et humanise votre profil. Format
                        JPG ou PNG, max 5Mo.</p>
                    <input type="file" name="profile_photo" accept="image/*"
                        class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100 transition">
                </div>
            </div>
            <div class="grid sm:grid-cols-2 gap-6">
                ...
                <div x-data="{ showNewSpecialization: false }">
                    ...
                    <select name="specialization_id" required
                        ...
                        <option value="{{ $spec->id }}" {{ old('specialization_id', $profile?->specialization_id) ==
                            $spec->id ? 'selected' : '' }}>
                            {{ $spec->name }}
                        </option>
                        @endforeach
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm">
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="is_published" value="1" {{ old('is_published', $profile->is_published ??
                false) ? 'checked' : '' }}
