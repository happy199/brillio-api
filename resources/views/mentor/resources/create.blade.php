...
<!-- Ciblage Avancé -->
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-6 sticky top-24">
                    ...
                    <div class="space-y-5 pt-5 border-t border-gray-100" x-data="{
                            ...
                            handleFileChange(event) {
                                const file = event.target.files[0];
                                if (file) {
                                    // Validation Taille (ex: 20MB pour correspondre au backend)
                            ...
                            <div class="relative">
                                ...
                                <label for="file_input" x-show="!uploadSuccess"
                                    ...
                                    <div x-show="!isUploading && !uploadSuccess" class="flex flex-col items-center">
                                        ...
                                        <span x-show="!uploadError" class="text-[9px] text-gray-400 mt-1">Docs, Images, Vidéos, Zip</span>
                                ...
                                <input id="file_input" type="file" name="file"
                                    accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.zip,.rar,.7z,.jpg,.jpeg,.png,.webp,.mp4,.webm,.mov"
                ...
                <p class="text-[10px] text-gray-500 mt-3 text-center leading-relaxed">
                    <svg class="w-3 h-3 inline-block mr-1 -mt-0.5 text-indigo-400" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    {{ isset($current_organization) ? $current_organization->name : 'Brillio' }} se réserve le droit de valider ou invalider toutes les ressources proposées afin d'en
                    garantir la qualité. Cette vérification se fait généralement dans les 4H après soumission.
                </p>
            </div>
        </div>
</div>
