                                                    ...
                                                    <div class="pl-4 space-y-4 border-l-2 border-gray-100">
                                                        ...
                                                        <template x-for="(option, optIndex) in question.options" :key="optIndex">
                                                            <div class="flex flex-col gap-1 group">
                                                                ...
                                                                <div class="pl-6 pr-8">
                                                                    <label :for="'edit_explanation_' + qIndex + '_' + qsIndex + '_' + optIndex" class="sr-only">Explication</label>
                                                                    <input :id="'edit_explanation_' + qIndex + '_' + qsIndex + '_' + optIndex" type="text" x-model="option.explanation" placeholder="Explication (optionnelle) : Pourquoi cette réponse est-elle vraie ou fausse ?" class="w-full text-xs bg-gray-50 border-gray-200 text-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 p-1.5 italic">
                                                                </div>
                                                            </div>
                                                        </template>
                    ...
                    <div class="space-y-5 pt-5 border-t border-gray-100">
                        ...
                        <div x-data="{ fileName: '' }">
                            ...
                            <label for="file_input"
                                class="flex flex-col items-center justify-center w-full h-20 border border-gray-200 rounded-lg cursor-pointer bg-white hover:bg-gray-50 transition overflow-hidden px-4">
                                <template x-if="!fileName">
                                    ...
                                    <div class="flex items-center gap-2 text-green-600">
                                        ...
                                        <span class="text-xs font-bold text-center">Fich. existant<br><span class="text-[10px] font-normal text-gray-500">Cliquez pour remplacer</span></span>
                                    </div>
                                    @else
                                    <span class="text-xs text-gray-600 font-medium text-center">Choisir un fichier...<br><span class="text-[10px] font-normal text-gray-400">PDF, ZIP, MP4...</span></span>
                                    @endif
                                </template>
                                <template x-if="fileName">
                                    <div class="flex items-center gap-2 text-indigo-600 w-full justify-center">
                                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                        <span class="text-xs font-bold truncate" x-text="fileName"></span>
                                    </div>
                                </template>
                                <input id="file_input" type="file" name="file" class="hidden" @change="fileName = $event.target.files[0] ? $event.target.files[0].name : ''" />
                            </label>
                        </div>
...
<script nonce="{{ request()->attributes->get('csp_nonce') }}">
    ...
    if (!oldQuizzes && {!! $resource->quizzes->count() > 0 ? 'true' : 'false' !!}) {
        @php
            $quizzes = $resource->quizzes->map(function($q) {
                return [
                    'id' => $q->id,
                    'title' => $q->title,
        ...
        @endphp
        oldQuizzes = {!! json_encode($quizzes) !!};
    }
</script>
@endpush