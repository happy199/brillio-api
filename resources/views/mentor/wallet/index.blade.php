...
<div x-data="{ activeTab: 'earnings' }" class="space-y-8">
    ...
    <div x-show="activeTab === 'earnings'" class="space-y-6">
        ...
        <div x-data="{ 
                                                            ...
                                                            // Calcul dynamique
                                                            get estimatedFee() {
                                                                const amount = parseInt(this.payoutForm.amount) || 0;
                                                                const fee = Math.max(amount * (this.feePercentage / 100), this.minFee);
            ...
            <div x-show="showPayoutModal" x-cloak
                ...
                <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" @click.stop>
                    ...
                    <form @submit.prevent="requestPayout" class="space-y-4">
                        ...
                        <div class="bg-blue-50 border border-blue-100 rounded-lg p-3 flex gap-3 items-start">
                            ...
                            <div>
                                <p class="text-xs text-blue-800">
                                    Des frais de service de <span class="font-bold"><span
                                            x-text="feePercentage"></span>%</span> sont appliqués sur chaque retrait
                                    pour couvrir les coûts de la transaction.
                                </p>
                            </div>
                        </div>
                        ...
                        <!-- Warning pour retrait manuel (Hors Bénin) -->
                        <div x-show="payoutForm.country_code && payoutForm.country_code !== 'BJ'"
                            class="bg-orange-50 border border-orange-200 rounded-lg p-3 flex gap-3 items-start">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="font-medium text-gray-900">{{ $transaction->related->user->name ??
                                            'Utilisateur Inconnu' }}</span>
                                        <span class="text-xs text-gray-500">Acheté le
                                            {{ $transaction->related->created_at->format('d/m/Y') }}</span>
                                    </div>
                                </div>
                                @elseif($transaction->related && $transaction->related instanceof
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="font-medium text-gray-900">{{
                                            $transaction->related->mentees->first()->name ?? 'Menté Inconnu' }}</span>
            ...
            <div class="p-4 bg-gray-50 border-t border-gray-100">
                {{ $incomeTransactions->appends(['wallet_page' => request()->wallet_page, 'active_tab' =>
                'earnings'])->links() }}
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden" x-data="{
            ...
            <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                ...
                <span
                    class="bg-emerald-100 text-emerald-800 text-xs font-bold px-2.5 py-0.5 rounded-full">Retraits</span>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-gray-600">
                    <thead class="bg-gray-50 text-xs uppercase text-gray-500">
    ...
    <div x-show="activeTab === 'recharge'" class="space-y-6" style="display: none;">
        ...
        <div
            ...
            <div>
                ...
                <div
                    class="mt-3 flex gap-2 text-xs font-semibold bg-white/10 p-2 rounded-lg items-center justify-between">
                    <div class="flex gap-4 items-center">
                        <div class="flex flex-col">
                            <span class="text-white/70 uppercase text-[10px]">Achetés</span>
                            <span class="text-white text-sm">{{ number_format($creditBreakdown['purchased']) }}</span>
                        </div>
                        <span class="text-white/50 text-lg font-light">+</span>
                        <div class="flex flex-col">
                            <span class="text-white/70 uppercase text-[10px]">Gagnés</span>
                            <span class="text-white text-sm">{{ number_format($creditBreakdown['earned']) }}</span>
                        </div>
                    </div>

                    <span class="text-white/50 text-xl font-light mx-1">-</span>

                    <div class="flex gap-4 items-center">
                        <div class="flex flex-col items-end">
                            <span class="text-white/70 uppercase text-[10px]">Dépensé</span>
                            <span class="text-white text-sm">{{ number_format($creditBreakdown['spent']) }}</span>
                        </div>
                        <span class="text-white/50 text-lg font-light">+</span>
                        <div class="flex flex-col items-end">
                            <span class="text-white/70 uppercase text-[10px]">Retiré</span>
                            <span class="text-white text-sm">{{ number_format($creditBreakdown['withdrawn']) }}</span>
                        </div>
                    </div>
                </div>
                @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-xl border border-gray-200 overflow-hidden sticky top-24">
                    ...
                    <div class="divide-y divide-gray-100 max-h-[500px] overflow-y-auto">
                        ...
                        <div class="p-4 hover:bg-gray-50 transition">
                            <div class="flex justify-between items-center mb-1">
                                <span class="text-xs font-medium text-gray-500">{{ $transaction->created_at->format('d M
                                    H:i') }}</span>
                                <span
                    ...
                    <!-- Pagination -->
                    <div class="p-4 bg-gray-50 border-t border-gray-100">
                        {{ $walletTransactions->appends(['income_page' => request()->income_page, 'active_tab' =>
                        'recharge'])->links('pagination.simple-orange') }}
                    </div>
                </div>
            </div>
        </div>
