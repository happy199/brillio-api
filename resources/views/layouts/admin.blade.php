...
<body class="bg-gray-100" x-data="{ sidebarOpen: false }">
    <div class="min-h-screen flex">
        ...
        <aside class="bg-indigo-700 text-white w-64 min-h-screen flex-shrink-0 hidden md:block">
            ...
            <nav class="mt-4">
                ...
                <!-- Commerciaux Dropdown -->
                @if(auth()->user()->isAdmin())
                <div
                    x-data="{ open: {{ request()->routeIs('admin.commercials.*') || request()->routeIs('admin.commercial_activities.*') ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                        class="w-full flex justify-between items-center px-4 py-3 hover:bg-indigo-600 {{ request()->routeIs('admin.commercials.*') || request()->routeIs('admin.commercial_activities.*') ? 'bg-indigo-800' : '' }}">
                        <span class="flex items-center">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div x-show="open" x-cloak class="bg-indigo-900">
                        <a href="{{ route('admin.commercials.index') }}"
                            class="block px-4 py-2 text-sm hover:bg-indigo-800 pl-12 text-indigo-200 hover:text-white {{ request()->routeIs('admin.commercials.index') ? 'text-white font-bold' : '' }}">
                            Annuaire des Commerciaux
                        </a>
                        <a href="{{ route('admin.commercial_activities.index') }}"
                            class="block px-4 py-2 text-sm hover:bg-indigo-800 pl-12 text-indigo-200 hover:text-white {{ request()->routeIs('admin.commercial_activities.index') ? 'text-white font-bold' : '' }}">
                ...
                <a href="{{ route('admin.careers.index') }}"
                    class="block px-4 py-3 hover:bg-indigo-600 {{ request()->routeIs('admin.careers.*') ? 'bg-indigo-800' : '' }}">
                    <span class="flex items-center">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                ...
                <a href="{{ route('admin.advertisements.index') }}"
                    class="block px-4 py-3 hover:bg-indigo-600 {{ request()->routeIs('admin.advertisements.*') ? 'bg-indigo-800' : '' }}">
                    <span class="flex items-center">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="mt-4 pt-4 border-t border-indigo-600">
                    <a href="{{ route('admin.two_factor.setup') }}"
                        class="block px-4 py-3 hover:bg-indigo-600 {{ request()->routeIs('admin.two_factor.*') ? 'bg-indigo-800' : '' }}">
                        <span class="flex items-center">
                            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-data="{ open: false, url: '', summary: '', loading: false }"
         @open-close-activity-modal.window="open = true; url = $event.detail.url; summary = ''; loading = false;"
         x-show="open" 
         x-cloak 
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="open" 
                ...
                <form :action="url" method="POST" @submit="loading = true">
                    @csrf
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div class="mt-4">
                                    <label for="summary" class="block text-sm font-medium text-gray-700">Compte rendu (obligatoire, min 10 caractères)</label>
                                    <textarea id="summary" name="summary" rows="4" x-model="summary" required minlength="10"
                                              class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" 
                                              placeholder="Veuillez résumer l'intervention..."></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button type="submit" :disabled="summary.length < 10 || loading"
                                class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50">
                            <span x-show="!loading">Enregistrer et Clôturer</span>
                            <span x-show="loading"><i class="fas fa-spinner fa-spin mr-2"></i>Chargement...</span>
                        </button>
