...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div
        ...
        <div>
            ...
            <p class="text-gray-500 flex items-center mt-1">
                ...
                {{ $session->full_scheduled_at_with_gmt }} ({{ $session->duration_minutes }} min)
            </p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden" x-data="{ showTranscription: false, showVideo: false }">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="border-t border-gray-100 pt-4">
                <h4 class="text-sm font-medium text-gray-500 mb-2">Évaluations des Jeunes / Participants</h4>
                @if($session->evaluations && $session->evaluations->isNotEmpty())
                    <div class="space-y-3">
                        @foreach($session->evaluations as $eval)
                            <div class="bg-amber-50/60 border border-amber-200/70 p-4 rounded-lg space-y-1.5">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        <span class="font-bold text-gray-900 text-xs">{{ $eval->mentee->name ?? 'Jeune' }}</span>
                                        <div class="flex items-center text-amber-400 gap-0.5 text-xs">
                                            ...
                                            @endfor
                                            <span class="ml-1 font-bold text-gray-800 text-xs">{{ $eval->rating }}/5</span>
                                        </div>
                                    </div>
                                    <span class="text-[11px] text-gray-400">{{ $eval->created_at->format('d/m/Y H:i') }}</span>
                                </div>
                                @if($eval->comment)
                                    <p class="text-xs text-gray-700 italic bg-white p-2.5 rounded border border-amber-100 whitespace-pre-wrap">"{{ trim($eval->comment) }}"</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
                @else
                    <p class="mt-1 text-gray-400 italic text-xs">Aucune évaluation enregistrée pour le moment.</p>
                @endif
            </div>
        </div>
