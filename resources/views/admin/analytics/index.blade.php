...
<div class="mb-24">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <!-- Liste des Commentaires avec Scroll Infini -->
            <div class="lg:col-span-2" x-data="{
                ...
                initialLoaded: false,
                async loadMore() {
                    if (this.loading || (!this.hasMore && this.initialLoaded)) return;
                    this.loading = true;
                    try {
                        const res = await fetch(`{{ route('admin.analytics.comments') }}?page=${this.page}`);
                ...
                <div class="flex justify-between items-center mb-4">
                    <h4 class="text-sm font-semibold text-gray-700">Derniers commentaires reçus</h4>
                    <span class="text-xs text-indigo-600 font-medium" x-show="comments.length > 0" x-text="`${comments.length} commentaire(s) chargé(s)`"></span>
                </div>

                <div class="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar" @scroll="onScroll($event)">
                    <template x-for="item in comments" :key="item.id">
                        <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 transition-all hover:border-gray-200">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold text-xs" x-text="item.initial"></div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-900" x-text="item.user_name"></p>
                                        <div class="flex items-center gap-1">
                                            <template x-for="star in 5" :key="star">
                                                <svg class="w-3 h-3" :class="star <= item.rating ? 'text-yellow-400' : 'text-gray-300'" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                </svg>
                                            </template>
                                            <span class="text-xs text-gray-400 ml-2" x-text="item.created_at_formatted"></span>
                                        </div>
                                    </div>
                                </div>
                                <span class="bg-white border shadow-sm px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider text-gray-500" x-text="item.user_type"></span>
                            </div>
                            <p class="text-sm text-gray-700 italic mt-3 bg-white p-3 rounded-lg border border-gray-100" x-text="`&quot;${item.comment}&quot;`"></p>
                        </div>
                    </template>
                    ...
                    <div x-show="initialLoaded && comments.length === 0" class="flex flex-col items-center justify-center h-32 text-gray-400 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                        <svg class="w-8 h-8 mb-2 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
                        <p class="text-sm">Aucun commentaire textuel reçu</p>
                    </div>

                    <div x-show="hasMore && !loading && comments.length > 0" class="text-center pt-2">
                        <button @click="loadMore()" type="button" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-4 py-2 rounded-lg transition-colors">
