            ...
            <div class="flex items-center">
                <div class="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-lg">
                    @if($user->profile_photo_url)
                        <img src="{{ $user->profile_photo_url }}" alt="" class="w-20 h-20 rounded-full object-cover">
                    @else
                ...
                <div class="ml-6 text-white">
                    ...
                    <div class="flex items-center mt-2 space-x-2">
                        <span class="px-2 py-1 text-xs rounded-full {{ $user->user_type === 'mentor' ? 'bg-purple-200 text-purple-800' : 'bg-blue-200 text-blue-800' }}">
                ...
                <div>
                    ...
                    <dl class="space-y-3">
                        <div class="flex justify-between">
                            ...
                            <dd class="text-gray-900">{{ $user->birth_date ? $user->birth_date->format('d/m/Y') : '-' }}</dd>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            ...
                            <dd class="text-gray-900">{{ $user->last_login_at ? $user->last_login_at->format('d/m/Y à H:i') : 'Jamais' }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="bg-orange-50 rounded-lg p-4 text-center">
                            ...
                            <p class="text-sm text-orange-600">Test MBTI</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
            ...
            @if($user->personalityTest->completed_at)
                <div class="flex items-center space-x-6">
                    <div class="w-24 h-24 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-xl flex items-center justify-center">
                        <span class="text-2xl font-bold text-white">{{ $user->personalityTest->personality_type }}</span>
                    </div>
                    <div class="flex-1">
                        <h4 class="text-xl font-semibold text-gray-900">{{ $user->personalityTest->personality_label ?? $user->personalityTest->personality_type }}</h4>
                        <p class="text-gray-600 mt-1">{{ Str::limit($user->personalityTest->personality_description, 200) }}</p>
                        <p class="text-sm text-gray-400 mt-2">Test passé le {{ $user->personalityTest->completed_at->format('d/m/Y à H:i') }}</p>
                    </div>
                </div>
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">{{ $document->document_type }} - {{ number_format($document->file_size / 1024, 1) }} Ko</p>
                            </div>
                        </div>
                        <a href="{{ route('admin.documents.download', $document) }}" class="text-indigo-600 hover:text-indigo-900">
                            Télécharger
                        </a>
                    </div>
                @endforeach
                ...
                @foreach($user->chatConversations()->with('messages')->latest()->take(5)->get() as $conversation)
                    <a href="{{ route('admin.chat.show', $conversation) }}"
                       class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                        <div>
                            <p class="font-medium text-gray-900">{{ $conversation->title ?? 'Conversation #' . $conversation->id }}</p>
        ...
        <div class="flex flex-wrap gap-3">
            @if($user->id !== auth()->id())
                <form action="{{ route('admin.users.toggle-admin', $user) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <button type="submit" class="px-4 py-2 {{ $user->is_admin ? 'bg-yellow-600' : 'bg-indigo-600' }} text-white rounded-lg hover:opacity-90">
