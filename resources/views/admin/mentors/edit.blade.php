...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            <button @click="$dispatch('open-linkedin-reload')" type="button"
                class="px-4 py-2 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg hover:bg-blue-100 transition flex items-center gap-2">
    ...
    <form id="main-mentor-form" action="{{ route('admin.mentors.update', $mentor) }}" method="POST" class="hidden">
        @csrf
        @method('PUT')
    </form>
    ...
    <div class="space-y-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">URL Photo de profil
                                (Source)</label>
                            <input type="url" name="profile_photo_url" form="main-mentor-form"
                                value="{{ old('profile_photo_url', $mentor->user->profile_photo_url) }}"
                                placeholder="https://media.licdn.com/..."
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            <p class="text-xs text-gray-500 mt-1">Si renseignée, l'image sera téléchargée et stockée
                                localement lors de la sauvegarde.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="bio" rows="4" form="main-mentor-form"
                        ...
                        <div>
                            ...
                            <textarea name="advice" rows="3" form="main-mentor-form"
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="current_position" form="main-mentor-form"
                                    value="{{ old('current_position', $mentor->current_position) }}"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Entreprise actuelle</label>
                                <input type="text" name="current_company" form="main-mentor-form"
                                    value="{{ old('current_company', $mentor->current_company) }}"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Années d'expérience</label>
                                <input type="number" name="years_of_experience" form="main-mentor-form"
                                    value="{{ old('years_of_experience', $mentor->years_of_experience) }}" min="0"
                                    max="60"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="specialization_id" form="main-mentor-form"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                                    <option value="">Aucune</option>
                                    @foreach($specializations as $spec)
                                    <option value="{{ $spec->id }}" {{ old('specialization_id', $mentor->
                                        specialization_id) == $spec->id ? 'selected' : '' }}>
                                        {{ $spec->name }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="grid md:grid-cols-2 gap-3">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Institution /
                                        Entreprise</label>
                                    <input type="text" name="institution_company"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                </div>
                                <div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="border border-gray-200 rounded-lg p-4 hover:border-indigo-300 transition">
                                ...
                                <div class="flex justify-between items-start">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2 mb-2">
                                            <span
                                                class="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-xs rounded font-medium">
                                    ...
                                    <div class="flex gap-2 ml-4">
                                        ...
                                        <form action="{{ route('admin.mentors.roadmap.delete', [$mentor, $step]) }}"
                                            method="POST" onsubmit="return confirm('Supprimer cette étape ?')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <div x-show="editingStep === {{ $step->id }}" x-transition>
                                <form action="{{ route('admin.mentors.roadmap.update', [$mentor, $step]) }}"
                                    method="POST" class="space-y-3">
                                    @csrf
                                    @method('PUT')
                                    <div class="grid md:grid-cols-2 gap-3">
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Type d'étape
                                                *</label>
                                            <select name="step_type" required
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                                @foreach(\App\Models\RoadmapStep::STEP_TYPES as $key => $label)
                                                <option value="{{ $key }}" {{ $step->step_type === $key ? 'selected' :
                                                    '' }}>{{ $label }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Institution /
                                                Entreprise</label>
                                            <input type="text" name="institution_company"
                                                value="{{ $step->institution_company }}"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                        </div>
                                        <div>
                                            <label
                                                class="block text-sm font-medium text-gray-700 mb-1">Localisation</label>
                                            <input type="text" name="location" value="{{ $step->location }}"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Date de début
                                                *</label>
                                            <input type="date" name="start_date"
                                                value="{{ $step->start_date?->format('Y-m-d') }}" required
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Date de
                                                fin</label>
                                            <input type="date" name="end_date"
                                                value="{{ $step->end_date?->format('Y-m-d') }}"
                                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                        </div>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                                        <textarea name="description" rows="2"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">{{ $step->description }}</textarea>
                                    </div>
                                    <div class="flex justify-end gap-2">
                                        <button type="button" @click="editingStep = null"
                                            class="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg text-sm">
                                            Annuler
                                        </button>
                                        <button type="submit"
                ...
                <div class="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="url" name="linkedin_url" form="main-mentor-form"
                                value="{{ old('linkedin_url', $mentor->linkedin_url) }}"
                        ...
                        <div>
                            ...
                            <input type="url" name="website_url" value="{{ old('website_url', $mentor->website_url) }}"
                                form="main-mentor-form" placeholder="https://..."
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div class="text-center mb-4">
                        ...
                        @else
                        <div
                            class="w-32 h-32 rounded-full mx-auto bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-4xl font-bold">
                            {{ substr($mentor->user->name, 0, 1) }}
                        </div>
                        @endif
                    </div>

                    <form action="{{ route('admin.mentors.update-photo', $mentor) }}" method="POST"
                    ...
                    @if($mentor->user->avatar_url)
                    <form action="{{ route('admin.mentors.delete-photo', $mentor) }}" method="POST" class="mt-3"
                        onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette photo ?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="w-full px-4 py-2 text-red-600 bg-red-50 border border-red-200 rounded-lg hover:bg-red-100 transition">
                            Supprimer la photo
                        </button>
                    </form>
                    @endif
                ...
                <div class="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center gap-3">
                            <input type="checkbox" name="is_published" value="1" form="main-mentor-form" {{
                        ...
                        <label class="flex items-center gap-3">
                            <input type="checkbox" name="is_validated" value="1" form="main-mentor-form" {{
        ...
        <div class="flex items-center justify-between bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" form="main-mentor-form"
    ...
    <script nonce="{{ request()->attributes->get('csp_nonce') }}" src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js" defer></script>
    @endsection