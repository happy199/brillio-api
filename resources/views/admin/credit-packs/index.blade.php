...
<div x-data="{ 
        ...
        organizationBasePrice: {{ $organizationCreditPrice }},
        formData: {
            id: '',
            user_type: '',
            name: '',
        ...
        get theoreticalPrice() {
            const prices = {
                jeune: this.jeuneBasePrice,
                mentor: this.mentorBasePrice,
                organization: this.organizationBasePrice
            };
            const base = prices[this.formData.user_type] || 0;
        ...
        openCreateModal(type) {
            this.editMode = false;
            const label = {jeune: 'Jeune', mentor: 'Mentor', organization: 'Organisation'}[type];
            this.modalTitle = 'Créer un Pack de Crédits (' + label + ')';
    ...
    <div class="border-b border-gray-200 mb-6">
        <nav class="-mb-px flex space-x-8">
            ...
            <button @click="activeTab = 'organization'"
                :class="activeTab === 'organization' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                Packs Organisation
            </button>
        </nav>
    ...
    <div x-show="activeTab === 'organization'" class="space-y-4" style="display: none;">
        ...
        <div class="bg-white shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($organizationPacks as $pack)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $pack->display_order }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $pack->name ?? '-' }}</div>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-center text-gray-500">Aucun pack Organisation trouvé.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
                ...
                <form :action="formAction" method="POST">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            <!-- Description -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Description / Avantages
                                    (Optionnel)</label>
                                <textarea name="description" x-model="formData.description" rows="3"
                                    placeholder="Ex: 50 crédits pour vos premières séances..."
