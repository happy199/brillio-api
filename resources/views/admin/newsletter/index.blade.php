        ...
        <form method="GET" class="flex flex-wrap gap-4 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}" 
                    placeholder="Email..." class="w-full px-4 py-2 border rounded-lg">
            </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 text-sm text-gray-600">{{ $subscriber->subscribed_at->format('d/m/Y H:i') }}</td>
                    <td class="px-6 py-4 text-sm space-x-3">
                        <button onclick="editSubscriber({{ $subscriber->id }}, '{{ $subscriber->email }}', '{{ $subscriber->status }}')" 
    ...
    <div class="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b bg-gradient-to-r from-blue-600 to-purple-600 text-white">
            <div class="flex items-center justify-between">
                ...
                <button onclick="document.getElementById('emailModal').classList.add('hidden')" class="text-white hover:text-gray-200">
        ...
        <form action="{{ route('admin.newsletter.send-email') }}" method="POST" class="p-6 space-y-6" id="emailForm">
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div class="space-y-3">
                    <label class="flex items-start cursor-pointer p-3 bg-white rounded border hover:border-blue-500">
                        <input type="radio" name="recipient_type" value="all" checked class="mt-1 mr-3" onchange="updateRecipientCount()">
                        <div>
                    ...
                    <label class="flex items-start cursor-pointer p-3 bg-white rounded border hover:border-blue-500">
                        <input type="radio" name="recipient_type" value="selected" class="mt-1 mr-3" onchange="updateRecipientCount()">
                        <div>
                            <span class="font-medium">Sélection personnalisée</span>
                            <p class="text-sm text-gray-600">Cochez les abonnés dans le tableau</p>
                        </div>
                    </label>
                </div>
            ...
            <div class="bg-gray-50 border rounded-lg p-4">
                ...
                <div class="flex gap-4">
                    <label class="flex items-center cursor-pointer p-3 bg-white rounded border hover:border-gray-400">
                        <input type="radio" name="format" value="html" checked class="mr-2" onchange="toggleFormatHelp()">
                        <span><i class="fas fa-code mr-1"></i>HTML (avec mise en forme)</span>
                    </label>
                    <label class="flex items-center cursor-pointer p-3 bg-white rounded border hover:border-gray-400">
                        <input type="radio" name="format" value="text" class="mr-2" onchange="toggleFormatHelp()">
            ...
            <div>
                ...
                <div id="htmlHelp" class="mb-2 p-3 bg-yellow-50 border border-yellow-200 rounded text-sm">
                    ...
                    <code class="bg-white px-1">&lt;strong&gt;</code>, <code class="bg-white px-1">&lt;a href=""&gt;</code>
                </div>
            ...
            <details class="border rounded-lg">
                <summary class="cursor-pointer p-3 bg-gray-50 font-medium hover:bg-gray-100">
...
&lt;/html&gt;</code></pre>
            ...
            <div class="flex justify-between items-center pt-4 border-t">
                ...
                <button type="submit" class="px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold rounded-lg hover:shadow-xl transform hover:scale-105 transition">
...
function updateRecipientCount() {
    ...
    document.getElementById('selectedCount').classList.toggle('hidden', type === 'all');
}
...
document.getElementById('emailForm').addEventListener('submit', function(e) {
    ...
    const count = type === 'all' ? {{ $stats['active'] }} : selected.length;
    if (!confirm(`📧 Confirmer l'envoi à ${count} destinataire(s) ?`)) {
        e.preventDefault();
        return;
    }
    ...
    recipientsInput.value = JSON.stringify(type === 'all' ? @json($subscribers->where('status', 'active')->pluck('email')) : selected);
