...
@section('title', 'Modifier le Métier')
...
@section('content')
    <div class="max-w-5xl mx-auto mb-24">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Modifier le Métier</h1>
                <p class="text-gray-500 text-sm mt-1">Édition de la fiche : <span
                        class="font-bold text-indigo-600">{{ $career->title }}</span></p>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.careers.update', $career->id) }}" method="POST" class="p-8 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text"
                            ...
                            id="demand_level" name="demand_level" value="{{ old('demand_level', $career->demand_level) }}"
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea
                            ...
                            placeholder="Pourquoi est-ce pertinent en Afrique aujourd'hui ?">{{ old('african_context', $career->african_context) }}</textarea>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text"
                            class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm transition-all h-12"
                            id="future_prospects" name="future_prospects"
                            value="{{ old('future_prospects', $career->future_prospects) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <select
                            ...
                            <option value="low" {{ old('ai_impact_level', $career->ai_impact_level) == 'low' ? 'selected' : '' }}>Faible (Stable)</option>
                            <option value="medium" {{ old('ai_impact_level', $career->ai_impact_level) == 'medium' ? 'selected' : '' }}>Moyen (Assisté par IA)</option>
                            <option value="high" {{ old('ai_impact_level', $career->ai_impact_level) == 'high' ? 'selected' : '' }}>Élevé (Secteur challengé)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea
                        ...
                        placeholder="Expliquez pourquoi ce niveau d'impact a été choisi...">{{ old('ai_impact_explanation', $career->ai_impact_explanation) }}</textarea>
                </div>
                        ...
                        <div class="grid grid-cols-4 gap-2">
                            ...
                            @foreach($mbtiList as $mbti)
                                <label
                                    ...
                                    <input type="checkbox" name="mbti_types[]" value="{{ $mbti }}" class="sr-only peer" {{ in_array($mbti, old('mbti_types', $mbtiTypes)) ? 'checked' : '' }}>
                                    <span
                            ...
                            @foreach($sectorsList as $sector)
                                <label
                                    ...
                                    <input type="checkbox" name="sectors[]" value="{{ $sector }}"
                                        class="rounded text-indigo-600 focus:ring-indigo-500 border-gray-300" {{ in_array($sector, old('sectors', $sectors)) ? 'checked' : '' }}>
    ...
    @push('scripts')
        <script nonce="{{ request()->attributes->get('csp_nonce') }}">
            document.getElementById('btn-generate-ai').addEventListener('click', function () {
                ...
                if (!confirm('Cela remplacera le contenu actuel par des suggestions de l\'IA. Continuer ?')) {
                    return;
                }
                ...
                fetch('{{ route("admin.ai.generate-career") }}', {
                    ...
                    .then(data => {
                        if (data.error) {
                            ...
                            successMsg.innerText = '✨ Contenu mis à jour par l\'IA !';
