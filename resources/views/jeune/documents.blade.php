...
<div class="space-y-8" x-data="documentsApp()">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm">
            ...
            <p class="text-2xl font-bold text-gray-900">{{ number_format($documents->sum('file_size') / 1024 / 1024, 1) }} Mo</p>
    ...
    <div class="flex flex-wrap gap-2">
        <button @click="filter = 'all'" :class="filter === 'all' ? 'bg-primary-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'"
                class="px-4 py-2 rounded-full text-sm font-medium transition">
            Tous
        </button>
        <button @click="filter = 'bulletin'" :class="filter === 'bulletin' ? 'bg-primary-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'"
                class="px-4 py-2 rounded-full text-sm font-medium transition">
            Bulletins
        </button>
        <button @click="filter = 'diplome'" :class="filter === 'diplome' ? 'bg-primary-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'"
                class="px-4 py-2 rounded-full text-sm font-medium transition">
            Diplomes
        </button>
        <button @click="filter = 'attestation'" :class="filter === 'attestation' ? 'bg-primary-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'"
                class="px-4 py-2 rounded-full text-sm font-medium transition">
            Attestations
        </button>
        <button @click="filter = 'autre'" :class="filter === 'autre' ? 'bg-primary-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'"
                class="px-4 py-2 rounded-full text-sm font-medium transition">
            Autres
        </button>
    </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm hover:shadow-md transition group"
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1 min-w-0">
                    <p class="font-semibold text-gray-900 truncate">{{ $document->original_name }}</p>
                    <p class="text-sm text-gray-500">{{ ucfirst($document->document_type) }}</p>
                    <div class="flex items-center gap-2 mt-2">
                        <span class="text-xs text-gray-400">{{ number_format($document->file_size / 1024, 1) }} Ko</span>
                        @if($document->is_verified)
                        <span class="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full flex items-center gap-1">
            ...
            <form action="{{ route('jeune.documents.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-4">
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-primary-500 transition cursor-pointer"
                        ...
                        <input type="file" name="document" x-ref="fileInput" class="hidden" accept=".pdf,.jpg,.jpeg,.png" @change="handleFileSelect($event)">
                    ...
                    <div>
                        ...
                        <select name="document_type" class="w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500">
