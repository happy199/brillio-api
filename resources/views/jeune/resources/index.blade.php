        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm space-y-4">
            ...
            <div class="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto flex-1 lg:justify-end">
                    <!-- Search -->
                    <form action="{{ route('jeune.resources.index') }}" method="GET" class="relative group w-full sm:w-64">
            ...
            <form action="{{ route('jeune.resources.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
                ...
                <!-- Ownership Toggle (Mine vs New) -->
                 <div class="bg-gray-100 rounded-lg p-1 flex text-xs font-medium">
                    <button type="submit" name="ownership" value="new" class="px-3 py-1.5 rounded-md transition {{ request('ownership', 'new') === 'new' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}" title="Ressources jamais consultées">
                        Nouveautés
                    </button>
                    <button type="submit" name="ownership" value="mine" class="px-3 py-1.5 rounded-md transition {{ request('ownership') === 'mine' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}" title="Ressources consultées ou achetées">
                        Mes Ressources
                    </button>
                ...
                <!-- Price Toggle -->
                <div class="bg-gray-100 rounded-lg p-1 flex text-xs font-medium">
                    <button type="submit" name="price" value="" class="px-3 py-1.5 rounded-md transition {{ !request('price') ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                        Tous prix
                    </button>
                    <button type="submit" name="price" value="free" class="px-3 py-1.5 rounded-md transition {{ request('price') === 'free' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                        Gratuit
                    </button>
                    <button type="submit" name="price" value="premium" class="px-3 py-1.5 rounded-md transition {{ request('price') === 'premium' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                        Payant
                    </button>
                </div>
                ...
                <div class="relative min-w-[200px]">
                            ...
                            @foreach($mbtiGroups as $group => $types)
                                <optgroup label="{{ $group }}">
                                    @foreach($types as $code => $label)
                                        <option value="{{ $code }}" {{ request('mbti') === $code ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </optgroup>
                            @endforeach
                        @else
                 ...
                 @if(request()->anyFilled(['search', 'type', 'price', 'mbti', 'source']))
                    <a href="{{ route('jeune.resources.index') }}" class="text-sm text-red-500 hover:text-red-700 underline ml-auto">
                        Réinitialiser
                    </a>
                 @endif
            </form>
            ...
            @if($resources->isEmpty())
                <div class="bg-white rounded-xl border border-gray-200 p-12 text-center">
                    ...
                    <p class="text-gray-500">Nous n'avons pas trouvé de ressources correspondant à tes filtres pour le moment. Essaie autre chose !</p>
                </div>
            @else
                            ...
                            <div class="p-5 flex-1 flex flex-col">
                                ...
                                <div class="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
                                    <div class="flex items-center gap-2">
                                        <div class="w-8 h-8 rounded-full bg-gray-200 overflow-hidden ring-2 ring-white shadow-sm flex-shrink-0">
                                        ...
                                        <div class="flex flex-col min-w-0">
                                            <span class="truncate font-medium text-gray-900">{{ $resource->user->name }}</span>
